...
export const authApi = {
  signIn: (email: string, password: string) => api.post("/auth/sign-in", { email, password }).catch(handleApiError),
...
export const communityApi = {
  ...
  joinCommunity: (communityId: number, nickName: string) =>
    api.post(`/communities/${communityId}/assign`, { nickName }).catch(handleApiError),
};
...
export const postApi = {
  ...
  getPosts: (isArtist: boolean, communityId: number, page = 1, limit = 10) =>
    api.get(`/posts`, { params: { isArtist, communityId, page, limit } }).catch(handleApiError),
...
export const merchandiseApi = {
  // 카테고리 조회 API
  fetchCategories: (communityId: number) => api.get(`/merchandise/category`, { params: { communityId } }),
  ...
  // 상품 상세 조회 API
  fetchMerchandiseDetail: (merchandiseId: number) => api.get(`/merchandise/${merchandiseId}`),

  // 장바구니 추가 API
  addToCart: (merchandiseId: number, merchandiseOptionId: number, quantity: number) =>
    api.post("/carts", { merchandiseId, merchandiseOptionId, quantity }).catch(handleApiError), // 공통 에러 처리 함수 사용
};
...
export const cartApi = {
  ...
  // 카트 항목 삭제 API
  removeCartItem: (cartItemId: number) => api.delete(`/carts/items/${cartItemId}`).catch(handleApiError),
};
